...
var playersOnline = [];
var PlayersActive = playersOnline.length + 1;
var chatCounter = 0;

const argLoggingUser = 'arglogger';
var argLoggerSocket = undefined;
...
const autoMessageTxt = 'Thank you for your participation in the experiment, your cloning will h e l p  u s';
...
io.on('connection', (socket) => {
  ...
  socket.on('login', (user) => {
    ...
    login(user.username, user.password, socket).then(function (res) {
      ...
      if(res.message.username == argLoggingUser) {
        argLoggingSocket = socket;
        console.log("arg logger connected");
      }
    })
    .catch(err => {
      console.error(err);
  ...
  socket.on('disconnect', function() {
    ...
    if(argLoggingSocket.id == socket.id) {
      argLoggingSocket = undefined;
      console.log("arg logger disconnected");
    }
  });
});
...
function doCommand(username, msg){
  ...
  if((command == 'playerCount')){
    var chatObj = {
        sender: 'System',
        message: `Current Player Count : ${PlayersActive}`,
        badgeColor: 'red'
    }
    socket.emit('chat', chatObj);
  }
...
async function modifyAccountSuspention(username, suspended, reason){
  ...
  if (suspended == false){
    ...
    setUser(userObj.username, userObj.password, defaultGameStats, newSuspentionStatus);
  }
  ...
  setTimeout(function(){
    ...
    if(hack.prevented == true){
      ...
      if(user.gameStats.otherUpgrades.covertHacks == false){
        const chatObj = {
          sender: 'Game',
          message: `${hackObj.hacker.username} has tried to clone ${hackObj.victim}, but was blocked.`,
          badgeColor: 'red'
        }
      ...
      if(user.gameStats.otherUpgrades.covertHacks == false){
        const chatObj = {
          sender: 'Game',
          message: `${hackObj.hacker.username} has successfully cloned $${formatNumber(hack.value)} from ${hackObj.victim}! y o u s h o u l d s t a r t r u n n i n g`,
          badgeColor: 'red'
        }
    ...
    if(argLoggingSocket){
      const hackInProgressObj = {
        hacker: hackObj.hacker.username,
        victim: hackObj.victim,
        money: hack.value,
        success: !hack.prevented
      }

      argLoggingSocket.emit('hackInProgress', hackInProgressObj)
    }
  }, firewallTime);
}
...
setInterval(function(){
  var OnlinePeople = playersOnline.length;
